...
if(!args[0]) return message.channel.send(` Bir zaman belirtmelisin!`)
        if(!args[0].endsWith("d")&&!args[0].endsWith("h")&&!args[0].endsWith("m")&&!args[0].endsWith("s")) return message.channel.send(` Yanlış zaman girdiniz!`)
        if(isNaN(args[0][0][0])) return message.channel.send(` Bu bir rakam değil!`)
        ...
        .addField('Zamanı İse', args[0] + ' Olarak Belirledi!')
        .setFooter(`Bitiş ->`)
